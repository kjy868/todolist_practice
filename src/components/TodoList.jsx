...
function TodoList({ todos, Delete, onToggle, onEdit, onUpdate }) {
    ...
    return (
        <div>
            {/* 목록 */}
            <h2 className="list-title">할 일 목록</h2>
            ...
            {
                // todos의 글자 길이가 0보다 길다<조건
                // 참일때: todos의 요소 찾기 item(글자), index(몇번째인지) 그래서 li에 글자로 순서대로 표기.
                // 거짓일때: 아직 할 일이 없어요 표기
                todosShow.length > 0 ?
                    <ul className='todo-list'>
                        {todosShow.map((item) => {
                            ...
                            {/* 수정중 목록 화면  */ }
                            const editView = (
                                <li key={item.id} id={item.id} className='edit-view'>
